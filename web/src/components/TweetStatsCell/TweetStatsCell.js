export const beforeQuery = ({ period = 'day', current = true }) => {
  return { variables: { period, current } }
}

export const QUERY = gql`
  query TweetStatsQuery($period: String!, $current: Boolean) {
    tweetStats(period: $period, current: $current) {
      date
      period
      delta
      total
...
export const Success = ({ tweetStats }) => {
  return (
    <div>
      <h3 className="text-lg leading-6 font-medium text-gray-900">Tweets</h3>
      {tweetStats.map((tweetStat, index) => {
        return (
          <div
            key={`tweet-stats-${index}`}
